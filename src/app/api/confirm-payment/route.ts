...
export async function POST(req: NextRequest) {
  try {
    ...
    // Expected payment details — allow either 0.2 USDC or 0.2 WLD
    const EXPECTED_RECIPIENT = "0xDd2a0c0EA69A77a99779f611A5cB97c63b215124".toLowerCase();
    const ACCEPTED = [
      { symbol: "USDC", amount: tokenToDecimals(0.2, Tokens.USDC).toString() },
      { symbol: "WLD", amount: tokenToDecimals(0.2, Tokens.WLD).toString() },
    ] as const;

    // Example optimistic verification via Developer Portal API  
    // Note: Using NEXT_PUBLIC_WLD_APP_ID as it's accessible both client/server side
    ...
    if (referenceOk && statusOk && recipientOk && tokenOk && amountOk) {
      // One-time use reference
      paymentReferences.delete(payload.reference);
